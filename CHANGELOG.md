# Changelog


## v1.1.3

[compare changes](https://github.com/vad1ym/ag-grid-vue3-slotted/compare/v1.1.2...v1.1.3)

### 🩹 Fixes

- Wrap slot nodes in display:contents div to render multiple root nodes ([8044011](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/8044011))

### ❤️ Contributors

- Vadym Bulakh ([@vad1ym](https://github.com/vad1ym))

## v1.1.2

[compare changes](https://github.com/vad1ym/ag-grid-vue3-slotted/compare/v1.1.0...v1.1.2)

### 🩹 Fixes

- Allow type safe custom columns slots by colId ([096dfec](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/096dfec))

### 🏡 Chore

- Add license ([e49ab85](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/e49ab85))

### ❤️ Contributors

- Vadym Bulakh ([@vad1ym](https://github.com/vad1ym))

## v1.1.0

[compare changes](https://github.com/vad1ym/ag-grid-vue3-slotted/compare/v1.0.1...v1.1.0)

### 🚀 Enhancements

- Add header cell text slot support ([7f25d04](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/7f25d04))

### 🏡 Chore

- Add example project ([5fa0496](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/5fa0496))
- Enhance readme ([d8fe2f3](https://github.com/vad1ym/ag-grid-vue3-slotted/commit/d8fe2f3))

### ❤️ Contributors

- Vadym Bulakh ([@vad1ym](https://github.com/vad1ym))

